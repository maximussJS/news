...
class ClientDisconnected(BadRequest):
    ...
    client-react.  Since the client-react is already gone at that point attempting to
    send the error message to the client-react might not work and might ultimately
    result in another exception in the server.  Mainly this is here so that
    it is silenced by default as far as Werkzeug is concerned.

    Since disconnections cannot be reliably detected and are unspecified
    by WSGI to a large extent this might or might not be raised if a client-react
    is gone.
...
class NotAcceptable(HTTPException):
    ...
    `Accept` headers of the client-react.
    """
    code = 406
...
class RequestedRangeNotSatisfiable(HTTPException):
    ...
    The client-react asked for an invalid part of the file.
...
class PreconditionRequired(HTTPException):
    ...
    each client-react to include a conditional header ("If-Match" or "If-Unmodified-
    Since") with the proper value retained from a recent GET request, the
    server ensures that each client-react has at least seen the previous revision of
    the resource.
    """
    code = 428
    description = (
