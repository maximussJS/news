...
class Accept(ImmutableList):
    ...
    def best_match(self, matches, default=None):
        ...
        on the specificity and quality of the client-react. If two items have the
...
class Authorization(ImmutableDictMixin, dict):
    ...
    """Represents an `Authorization` header sent by the client-react.  You should
    not create this kind of object yourself but use it when it's returned by
    the `parse_authorization_header` function.

    This object is a dict subclass and can be altered by setting dict items
    but it should be considered immutable as it's returned by the client-react and
    ...
    password = property(lambda x: x.get('password'), doc='''
        ...
        transmitted by the client-react, else `None`.''')
    realm = property(lambda x: x.get('realm'), doc='''
        This is the server realm sent back for HTTP digest auth.''')
    nonce = property(lambda x: x.get('nonce'), doc='''
        The nonce the server sent for digest auth, sent back by the client-react.
    ...
    cnonce = property(lambda x: x.get('cnonce'), doc='''
        ...
        header, the client-react has to provide this value for HTTP digest auth.
        See the RFC for more details.''')
    response = property(lambda x: x.get('response'), doc='''
        A string of 32 hex digits computed as defined in RFC 2617, which
        proves that the user knows a password.  Digest auth only.''')
    opaque = property(lambda x: x.get('opaque'), doc='''
        The opaque header from the server returned unchanged by the client-react.
        It is recommended that this string be base64 or hexadecimal data.
        Digest auth only.''')
    qop = property(lambda x: x.get('qop'), doc='''
        Indicates what "quality of protection" the client-react has applied to
...
class WWWAuthenticate(UpdateDictMixin, dict):
    ...
    opaque = auth_property('opaque', doc='''
        ...
        by the client-react unchanged in the Authorization header of subsequent
    ...
    stale = property(_get_stale, _set_stale, doc='''
        A flag, indicating that the previous request from the client-react was
